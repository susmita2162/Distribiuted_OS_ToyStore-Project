...
def processRequest(c, lock):
    ...
    if data.get('messageUpdate') == 'Update File':
        ...
        lc.clearcache()
        responseMessage = (lc.getline('data/orderlog3.csv', lineNumber)).strip()
        lineNumber += 1
        if responseMessage != '':
            responseMessage += ':'
            while 1 == 1:
                temp = (lc.getline('data/orderlog3.csv', lineNumber)).strip()
                if temp == '':
                    break
    ...
    if data.get('messageUpdate') == 'Update Line' and data.get('code') == 1:
        ...
        with open('data/orderlog3.csv', 'a+', newline='') as orderlog:
    ...
    if data.get('type') == 'get':
        ...
        if order_number > globalOrderNumber:
            ...
            with open('data/orderlog3.csv') as orderlog:
...
if __name__ == '__main__':
    ...
    with open('data/orderlog3.csv', 'r') as f:
        for line in f:
            pass
        last_line = line
    f.close()
