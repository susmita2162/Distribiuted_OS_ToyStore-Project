...
def processRequest(c, lock):
    ...
    if data.get('messageUpdate') == 'Update File':
        ...
        lc.clearcache()
        responseMessage = (lc.getline('data/orderlog1.csv', lineNumber)).strip()
        lineNumber += 1
        if responseMessage != '':
            responseMessage += ':'
            while 1 == 1:
                temp = (lc.getline('data/orderlog1.csv', lineNumber)).strip()
                if temp == '':
                    break
    ...
    if data.get('messageUpdate') == 'Update Line' and data.get('code') == 1:
        ...
        with open('data/orderlog1.csv', 'a+', newline='') as orderlog:
    ...
    if data.get('type') == 'get':
        ...
        if order_number > globalOrderNumber:
            ...
            with open('data/orderlog1.csv') as orderlog:
        ...
        if message.get("code") == 1:
            ...
            try:
                ...
                orderService.close()
                print('Order Service 9002 is down')
            try:
...
if __name__ == '__main__':
    ...
    with open('data/orderlog1.csv', 'r') as f:
        for line in f:
            pass
        last_line = line
    f.close()
